...
- **WHEN** a client GETs `/api/meta` with Resend secrets set, `VR_EMAIL_ALERTS` unset, and `VR_PASSWORD_RESET` unset
- **THEN** the body is `{ password_reset: false, email_alerts: false }`

#### Scenario: Meta reports password reset only when re-enabled
- **WHEN** a client GETs `/api/meta` with Resend secrets set and `VR_PASSWORD_RESET` truthy, and `VR_EMAIL_ALERTS` unset
- **THEN** the body is `{ password_reset: true, email_alerts: false }`


#### Scenario: Stored email channel does not send while upcoming
- **WHEN** `VR_EMAIL_ALERTS` is unset, Resend secrets are set, and a Check already has an email channel that would alert
- **THEN** the Worker does not POST to Resend for that alert
