...
- [x] 5.4 Blind review at `ccae3ed` (build at both SHAs, built-CSS class-order and purge checks, a server-rendered `Timeline` inside the row link, a 375 px Playwright probe of the rename edit state): **APPROVE**, 0 critical / high / medium, 1 low — "`{!latest && runHistory}` can no longer render anything … when `!latest` holds, `runHistory` is null" (`CheckDetail.jsx:333`). Fixed: the line is deleted; the live render at the top of the page stays
- [x] 5.5 PR #9 merged to `main` 2026-09-03 (`7ec16d3`); deploy workflow run 33753470945; `CLAIMS.md` row flipped to `done` in the archive commit
