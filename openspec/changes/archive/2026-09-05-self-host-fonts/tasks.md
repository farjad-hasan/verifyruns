...
- [x] 3.3 PR #14 merged 2026-09-05 (`c7e429e`); deploy run 33913406013 green. Live: CSP `style-src 'self' 'unsafe-inline'; font-src 'self'`, zero Google references in HTML and CSS, three woff2 served 200 from `static/media/`; in Edge on `/pricing`, `document.fonts` reports Outfit, Manrope and JetBrains Mono loaded and no resource host outside the site and the API. The Pages Web Analytics beacon is also gone from the HTML (disabled in the dashboard 2026-09-05). `CLAIMS.md` flipped, change archived, worktree removed
