...
## Purpose
Where the server is allowed to make outbound requests: public addresses only (checked at save time and before every fetch), bounded response size, and per-key rate limits on the abuse surfaces. Switchable for self-hosters who watch internal databases.
