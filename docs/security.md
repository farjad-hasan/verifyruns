...
- Postgres connector: single `SELECT`/`WITH` statement (no semicolons), wrapped as a subquery, with `default_transaction_read_only = on` on the session. Use a read-only role anyway.
