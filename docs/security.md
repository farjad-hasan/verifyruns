...
- Passwords (2026-08-31): PBKDF2-SHA256 via WebCrypto, 600,000 iterations (OWASP guidance), per-user 16-byte salt. Each stored hash carries its own iteration count; verification uses the stored count and a successful login against a weaker hash transparently re-hashes at the current strength — no bulk migration. Login performs the same PBKDF2 work for unknown emails as for known ones, so timing does not reveal whether an account exists.
