...
An error handler route (Break / Rollback / Ignore) can call the same webhook with `{"status": "failed", "error": "<the error message>"}`; that run is a FAIL with the reason and is alerted without a retry.
