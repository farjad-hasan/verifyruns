...
Zapier's own alerts tell you when a Zap *errors*; VerifyRuns tells you when a Zap *succeeds without landing anything*. Use both — or route the error path to the same webhook with `status` → `failed` and `error` → the message, so one status page carries both facts.
