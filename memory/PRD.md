...
- **API**: Cloudflare Worker + D1 (`worker/src`). All routes under `/api`.
...
- Tick latency + async (2026-08-28, Cloudflare-first): lazy tick on any request (atomic claim in `meta.tick`, 60 s window), `?wait=0` queues the run on the Check (`pending_runs`, 202) for the next tick, Airtable counts later pages with one `fields[]` and fetches the newest record by id when off page one; `deploy/cloudflare-tick-worker/` (Worker cron every minute) replaces cron-job.org as the recommended scheduler
