...
const POSTURE = [
  ...
  { title: "Alert delivery", body: "VerifyRuns attempts alerts on the first failure in a streak and on recovery. Provider acceptance does not prove receipt: if every channel fails, a durable notification stays queued and the scheduler retries without another workflow run. Interrupted acknowledgement can duplicate delivery. Pending delivery respects snooze and removed channels; each failed delivery is recorded on the run and counted in a service-wide number that holds no targets or message bodies. Alert webhooks never follow redirects." },
