...
4. **Test alerts and get verdicts** — use Send test and confirm receipt. — every run is PASS or FAIL with a diff message. Configured Slack and Discord channels are attempted on the first FAIL and on recovery. Email alerts are upcoming. Failed deliveries are retried by the scheduler without another workflow run. An interrupted acknowledgement can duplicate an alert. Failed deliveries are retried by the scheduler without another workflow run. An interrupted acknowledgement can duplicate an alert. Confirm actual receipt; a provider accepting the message is not a delivery guarantee.
